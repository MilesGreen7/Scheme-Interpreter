...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        evaluated_expr = scheme_eval(expressions.rest.first, env)
        env.define(signature, evaluated_expr)
        return signature
        # END PROBLEM 4
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    next_expression = expressions
    while next_expression.rest is not nil:
        eval_result = scheme_eval(next_expression.first, env)
        if is_scheme_false(eval_result):
            return eval_result
        next_expression = next_expression.rest
    return scheme_eval(next_expression.first, env, True)
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while next_binding != nil:
        validate_form(next_binding.first, 2, 2)
        names = Pair(next_binding.first.first, names)
        validate_formals(names)
        values = Pair(scheme_eval(next_binding.first.rest.first, env), values)
        next_binding = next_binding.rest
    # END PROBLEM 14
